...
本轮[32项验收记录](../../artifacts/bid-full-sample/real-run-v16-review/independent-acceptance.json)仍全部待全文核验，人工答案保留在模型来源目录外。[实际出件入口](../../artifacts/bid-full-sample/real-run-v16-review/output-readiness.json)已通过API原生测试入口编译，但尚未运行Office整稿验收。按[交接顺序](../../artifacts/bid-full-sample/real-run-v16-review/composition-handoff.json)先验提取断言，再生成并核对DOCX，最后更新目录并核对同版PDF；不能在生成前提前关闭需要实际产物的断言。
