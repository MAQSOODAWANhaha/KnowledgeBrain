...
- 公司证据检索只扫 `kind=company` Workspace 下 library 的 current，不把 library hit 记到产品线产品分数；招投标使用方式只由 [`../knowledge-base/domain.md`](../knowledge-base/domain.md) 的 `KnowledgeRetrievalPort` 定义。
...
- 文档进度：`document_processing_spans` 五段树（产品 timeline）。
- 队列：`oxana-web` `/ops/oxana`（积压，不是解析过程）。
- 进程日志：api/worker 用 `tracing` + `tracing-subscriber` 写 stdout；`RUST_LOG` 默认 `info`。解析/评估过程事件（convert、chunk、VLM、extract section）给 `docker logs`，不替代 spans 或 oxana 看板。
- 指标：Prometheus。
- 无 Langfuse。
