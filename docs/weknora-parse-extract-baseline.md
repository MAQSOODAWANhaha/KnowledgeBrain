...
| 本仓规格 | `docs/system-design.md`（知识库管线与 brain/WeKnora **同语义**） |
| 本仓投标 | `docs/bid-platform-domain.md` |
| 本仓改进方案 | `plans/parse-extract-weknora-parity.md`（已执行一轮；MinerU/Paddle **本期不部署**） |

本文只记录 **WeKnora 公开实现在快照日的机制**，以及当时对本仓「抽不全 / 内容不对」的对照结论。  
**不是** KnowledgeBrain 的运行规格；规格以 `docs/system-design.md` 为准。
...
证据范围（快照日）：WeKnora 公开仓库与文档。**没有**把 KnowledgeBrain 源码送进那次抓取；本仓差距以当时阅读 + 后续 `plans/parse-extract-weknora-parity.md` 为准。
...
| 日期 | WeKnora 锚点 | 本仓动作 |
|---|---|---|
| 2026-08-20 | main `ba03be77…`；release `v0.7.2` = `3d5d8bfc…` | 首次写入本文；方案 `plans/parse-extract-weknora-parity.md` |
