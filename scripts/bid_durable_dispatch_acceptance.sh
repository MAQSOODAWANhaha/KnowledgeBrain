...
run_logged runtime-jobs "$ARTIFACT_DIR/logs/runtime-jobs.log" \
  cargo test --locked -p platform jobs::tests -- --nocapture
run_logged work-transport "$ARTIFACT_DIR/logs/work-transport.log" \
  cargo test --locked -p platform --test work_transport -- --nocapture
run_logged work-transport-live "$ARTIFACT_DIR/logs/work-transport-live.log" \
  cargo test --locked -p platform --test work_transport_live -- --nocapture --test-threads=1
run_logged durable-delivery-sql "$ARTIFACT_DIR/logs/durable-delivery-sql.log" \
  cargo test --locked -p bidding --test durable_delivery_sql -- --nocapture --test-threads=1
...
run_logged tender-publication "$ARTIFACT_DIR/logs/tender-publication.log" \
  cargo test --locked -p bidding --test tender_publication -- --nocapture --test-threads=1
run_logged matching-publication "$ARTIFACT_DIR/logs/matching-publication.log" \
  cargo test --locked -p bidding --test matching_publication -- --nocapture --test-threads=1
run_logged submission-sql "$ARTIFACT_DIR/logs/submission-sql.log" \
  cargo test --locked -p bidding --test submission_sql -- --nocapture --test-threads=1
