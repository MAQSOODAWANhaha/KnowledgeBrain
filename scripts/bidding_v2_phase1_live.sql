...
BEGIN
  ...
  IF jsonb_array_length(documents)<>2 OR jsonb_array_length(relations)<>1
  ...
  IF (SELECT generation FROM bid_workspace_requirement_projection_current
      WHERE project_id='10000000-0000-4000-8000-000000000010')<>2 THEN
...
BEGIN
  ...
  SELECT * INTO STRICT docset FROM bid_document_set_current
  ...
  request_value:=kb_bid_v2_create_outline_candidate(
    ...
    docset.artifact_id,docset.artifact_sha256,actor,'phase2-outline-generate',request_bytes,request_sha);
  replay_value:=kb_bid_v2_create_outline_candidate(
    workspace_id,(workspace_value->>'revision_id')::uuid,(workspace_value->>'sha256')::kb_sha256,
    docset.artifact_id,docset.artifact_sha256,actor,'phase2-outline-generate',request_bytes,request_sha);
  IF request_value IS DISTINCT FROM replay_value OR request_value->>'status'<>'succeeded' THEN
    RAISE EXCEPTION 'outline generation replay/status mismatch';
  END IF;
  candidate_id:=(request_value#>>'{result_identity,artifact_id}')::uuid;
