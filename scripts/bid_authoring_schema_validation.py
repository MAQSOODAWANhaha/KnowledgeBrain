#!/usr/bin/env python3
"""Draft 2020-12 positive/negative tests for document and shared contracts.

Agent tool catalogs are validated by their runtime contract tests.
"""
...
def deref(ref: str, root: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    document_id, _, fragment = ref.partition("#")
    document = SCHEMAS_BY_ID[document_id] if document_id else root
    target = document
...
def sample(schema: dict[str, Any] | bool, root: dict[str, Any]) -> Any:
    if schema is True:
        return None
    if schema is False:
        raise AssertionError("cannot synthesize a value for false schema")
    if "$ref" in schema:
    ...
    if kind == "array":
        values = [sample(item, root) for item in schema.get("prefixItems", [])]
        while len(values) < schema.get("minItems", 0):
            values.append(sample(schema.get("items", True), root))
        return values
    if kind == "string":
        ...
        if schema.get("pattern") in {"^[0-9a-f]{64}$", "^[a-f0-9]{64}$"}:
            return SHA
        if schema.get("pattern") == "^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$":
            return UUID
...
def validator(
    ...
    if name == "content-generation-input-v1.schema.json":
        instance["agent_contract"] = None
...
def main() -> None:
    assert set(SCHEMAS) == CURRENT_SCHEMA_INVENTORY
