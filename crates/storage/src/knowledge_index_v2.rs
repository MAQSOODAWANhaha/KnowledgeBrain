...
use domain::knowledge_retrieval::EmbeddingRevisionV2;
...
impl VectorEmbeddingTransportV2 for ReqwestVectorEmbeddingTransportV2 {
    async fn send(
        ...
        let response = crate::knowledge_retrieval::http_v2::read_bounded_response_body_v2(
            response,
            self.max_response_bytes,
        )
        .await
        .map_err(|error| match error {
            crate::knowledge_retrieval::http_v2::BoundedBodyErrorV2::TooLarge => {
                VectorIndexErrorV2::Unavailable(
                    "embedding provider response exceeds byte limit".into(),
                )
            }
            crate::knowledge_retrieval::http_v2::BoundedBodyErrorV2::Transport(error) => {
                VectorIndexErrorV2::Unavailable(format!(
