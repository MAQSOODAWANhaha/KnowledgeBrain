...
pub const MIGRATION_0007: &str = include_str!("../../../migrations/0007_housekeep.sql");
...
pub async fn hydrate_workspace(
    ...
    let row = sqlx::query("SELECT id, name, slug, retrieval_config FROM workspaces WHERE id = $1")
        .bind(workspace_id)
        .fetch_optional(pool)
        .await?;
    let Some(r) = row else {
