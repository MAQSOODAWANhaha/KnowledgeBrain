...
pub fn extract_chunk_for_attempt(
    ...
    if enrichment::attempt_superseded(doc.attempt, job_attempt) {
    ...
    let raw = enrichment::chat_complete(&system, &user, &version.summary_model_id)?;
...
mod tests {
    ...
    fn extract_uses_effective_version_overrides() {
        ...
        doc.process_overrides = Some(domain::ProcessOverrides {
            extract_config: Some(domain::ExtractOverride {
                enabled: false,
                text: String::new(),
            }),
            ..Default::default()
