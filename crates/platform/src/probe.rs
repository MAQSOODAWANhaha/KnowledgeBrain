...
pub fn inspect_signed_launch_artifacts() -> Result<(), &'static str> {
    let registry_ok = crate::QueueRegistry::load().is_ok();
    let intended_ok = registry_ok && crate::IntendedState::load().is_ok();
...
fn chat_capability_ready() -> bool {
    let model = crate::chat_model();
    let model = model.trim();
    !crate::chat_base_url().is_empty() && !model.is_empty() && model != "stub-chat"
}

fn embedding_capability_ready() -> bool {
    let model = crate::embedding_model();
    let model = model.trim();
    !crate::embedding_base_url().is_empty() && !model.is_empty() && model != "stub-emb"
}
...
fn disabled_lanes_ready(intended: &crate::IntendedState, registry: &crate::QueueRegistry) -> bool {
    ...
    let expected_disabled = BTreeSet::from([
        crate::TYPE_IMAGE_MULTIMODAL,
        crate::TYPE_CHUNK_EXTRACT,
        crate::TYPE_QUESTION,
    ]);
    if intended_disabled != registry_disabled || intended_disabled != expected_disabled {
        return false;
    }
    [crate::TYPE_BID_DELIVERY_V1].into_iter().all(|task_type| {
        intended
            .lane_for_task(task_type)
            .is_some_and(|lane| lane.state == crate::FeatureState::Enabled)
    })
}

fn enabled_lane_lists_capability(
    intended: &crate::IntendedState,
    registry: &crate::QueueRegistry,
    capability: &str,
) -> Result<bool, &'static str> {
    for lane in intended.lanes() {
        if lane.state != crate::FeatureState::Enabled {
            continue;
        }
...
pub fn inspect_launch_runtime_policy() -> Result<(), &'static str> {
    let registry = crate::QueueRegistry::load().map_err(|_| REASON_DISABLED_LANE_MISMATCH)?;
    let intended = crate::IntendedState::load().map_err(|_| REASON_DISABLED_LANE_MISMATCH)?;
