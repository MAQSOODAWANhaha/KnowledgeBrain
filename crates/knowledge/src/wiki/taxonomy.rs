...
fn classify_one_batch(
    ...
    let raw = crate::enrichment::chat_complete_wiki(
...
pub fn cite_with_llm(
    ...
    if !crate::enrichment::chat_http_configured() {
...
pub fn page_content_for(
    ...
    if page_type == PAGE_SUMMARY {
        ...
        if !s.is_empty() {
            return clip_runes(s, crate::wiki::ASSEMBLE_RUNE_CAP);
        }
...
pub fn reduce_page(input: ReduceInput<'_>) -> (String, String) {
    ...
    if page_type == PAGE_SUMMARY || !crate::enrichment::chat_http_configured() {
    ...
    match crate::enrichment::chat_complete_wiki(
        &format!(
...
pub fn assemble_body(chunks: &[Chunk]) -> String {
    let mut body: String = chunks
        ...
        .collect::<Vec<_>>()
        .join("\n");
    if body.chars().count() > crate::wiki::ASSEMBLE_RUNE_CAP {
        body = body.chars().take(crate::wiki::ASSEMBLE_RUNE_CAP).collect();
    }
    body
}
...
fn select_folders_by_vectors(
    ...
    for iv in item_vecs {
        let mut ranking: Vec<(usize, f64)> = folder_vecs
            .iter()
            .enumerate()
            .map(|(i, fv)| (i, crate::index::cosine(iv, fv)))
            .collect();
...
pub fn nearest_folder(title: &str, about: &str, candidates: &[Vec<String>]) -> Option<Vec<String>> {
    ...
    let about: String = about.chars().take(120).collect();
    let iv = crate::index::embed(&format!("{title} {about}"));
    deeper.into_iter().max_by(|a, b| {
        let sa = crate::index::cosine(&iv, &crate::index::embed(&a.join(" / ")));
        let sb = crate::index::cosine(&iv, &crate::index::embed(&b.join(" / ")));
