...
use crate::Store;
use crate::search::{AssemblyResponse, Hit, SearchError, SearchRequest, assembly};
...
mod tests {
    ...
    use crate::{Document, Product, ProductKind, ProductVersion, Store, Workspace};
    use uuid::Uuid;
    ...
    fn seed_product(s: &mut Store) -> (Uuid, Uuid, Uuid) {
        let ws = Workspace {
            ...
            retrieval: crate::RetrievalConfig::default(),
        };
        let wid = ws.id;
