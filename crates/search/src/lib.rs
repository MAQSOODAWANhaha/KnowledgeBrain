...
use domain::{ProductKind, Store, VersionStatus};
use index::{cosine, embed_index, keyword_score};
...
fn requirements_from_tender(tender: &str, model_id: &str, tag_ids: &[Uuid]) -> Vec<Requirement> {
    ...
    let raw = enrichment::chat_complete(prompt, tender, model_id).unwrap_or_default();
...
pub async fn assembly_pg(
    ...
    let targets = storage::resolve_pg_assembly_targets(
        pool,
        product_id,
    ...
    let models = storage::embedding_models_for_versions(pool, &targets)
        .await
        .map_err(|e| SearchError {
            code: "INTERNAL",
            message: e.to_string(),
    ...
    let qv = storage::vector_literal(&qv);
    let limit = per_target_limit(req.match_count, targets.len()) as i64;
    let (vth, kth) = storage::workspace_thresholds_for_product(pool, product_id)
        .await
        .unwrap_or((0.15, 0.3));
    let mut hits = Vec::new();
    let mut warnings = Vec::new();
    ...
    for chunk in targets.chunks(4) {
        ...
        for vid in chunk {
            ...
            handles.push(tokio::spawn(async move {
                let rows = storage::hybrid_search_pg(&pool, vid, &q, &qv, &tags, wiki, limit).await;
                match rows {
                    Ok(rows) => {
...
fn hits_from_pg_rows(rows: Vec<storage::PgSearchHit>, vth: f64, kth: f64) -> Vec<Hit> {
    let mut hits = Vec::new();
    for row in rows {
...
async fn graph_hits_for_version(
    ...
    let rows = storage::graph_hits_pg(pool, version_id, query, match_count.max(1) as i64, tag_ids)
        .await
        .map_err(|e| SearchError {
            code: "INTERNAL",
            message: e.to_string(),
...
async fn pg_version_hits(
    ...
    let qv = storage::vector_literal(&qv);
    let rows = storage::hybrid_search_pg(
        pool,
        version_id,
        query,
        &qv,
    ...
    let (vth, kth) = storage::workspace_thresholds_for_version(pool, version_id)
        .await
        .unwrap_or((0.15, 0.3));
...
fn product_versions(store: &Store, p: &domain::Product, scope: &str) -> Vec<Uuid> {
    if scope == "all_active" {
        store
            .versions
            .values()
...
mod tests {
    ...
    async fn matching_pg_returns_candidates_without_best_product() {
        ...
        storage::insert_user(&pool, owner, &format!("{owner}@ex.com"), None)
            .await
            .unwrap();
        let seeded = storage::create_workspace_with_library(&pool, owner, "Mt", "mt")
            .await
            .unwrap();
        let pid = Uuid::new_v4();
        let vid = Uuid::new_v4();
        ...
        .await
        .unwrap();
        storage::insert_version(&pool, vid, pid, "v1", "active", None)
            .await
            .unwrap();
        let did = Uuid::new_v4();
