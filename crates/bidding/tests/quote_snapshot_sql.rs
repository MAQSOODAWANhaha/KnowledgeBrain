...
async fn rust_and_sql_canonical_bytes_match() {
    ...
    let line = compute_line(
        &QuoteLineInput {
            ...
            pricing_mode: bidding::quote::PricingMode::LumpSum,
            quantity: None,
            unit: None,
            unit_price: None,
...
async fn partial_unit_price_fields_persist_until_the_line_is_complete() {
    ...
    let response = bidding::bid_quote::upsert_quote_line(
        &pool,
        UpsertQuoteLine {
            project_id: seed.project_id,
            line_id,
    ...
    let state = bidding::bid_quote::quote_state(&pool, seed.project_id)
        .await
        .unwrap();
...
async fn quote_finalize_and_reopen_enforce_all_cas_identities() {
    ...
    let created = bidding::bid_quote::create_quote_draft(
        &pool,
        seed.project_id,
        "tax_inclusive",
        "投标报价一览表",
    ...
    let error = bidding::bid_quote::upsert_quote_line(
        &pool,
        UpsertQuoteLine {
            project_id: seed.project_id,
            line_id,
    ...
    let line = bidding::bid_quote::upsert_quote_line(
        &pool,
        UpsertQuoteLine {
            project_id: seed.project_id,
            line_id,
    ...
    let finalized_state = bidding::bid_quote::quote_state(&pool, seed.project_id)
        .await
        .unwrap();
    ...
    let reopened_state = bidding::bid_quote::quote_state(&pool, seed.project_id)
        .await
        .unwrap();
