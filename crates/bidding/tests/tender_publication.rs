use bidding::tender::outline_and_route;
...
async fn seed_publication(pool: &PgPool, markdown: &str) -> PublicationSeed {
    ...
    let markdown_sha256 = knowledge::sha256_hex(markdown.as_bytes());
...
async fn publication_is_atomic_and_fact_accept_race_is_linearizable() {
    ...
    let first = bidding::bidding::publish_extraction_section(
        &pool,
        PublishSection {
    ...
    let invalid = bidding::bidding::publish_extraction_section(
        &pool,
        PublishSection {
    ...
    let accept = bidding::bidding::mutate_fact(
        &pool,
        FactMutation {
            project_id: seed.project_id,
            action: "accept",
    ...
    let replace = bidding::bidding::publish_extraction_section(
        &pool,
        PublishSection {
            target_id: second_target_id,
            extraction_generation: 2,
    ...
    let project_fact_revision: i64 =
        sqlx::query_scalar("SELECT fact_revision FROM bid_projects WHERE id=$1")
            ...
            .await
            .unwrap();
    let history = bidding::bidding::fact_suggestion_history(&pool, seed.project_id)
        .await
        .unwrap();
    ...
    let current = bidding::bidding::current_fact_suggestions(&pool, seed.project_id)
        .await
        .unwrap();
...
async fn kind_router_promotions_refresh_generation_two_and_three_markers() {
    ...
    let publication = bidding::bidding::publish_extraction_section(
        &pool,
        PublishSection {
            target_id,
            extraction_generation: 1,
    ...
    .await
    .unwrap();
    let text_sha256 = knowledge::sha256_hex(clause_text.as_bytes());
    ...
    let reconfirm_sha256 = knowledge::sha256_hex(&reconfirm_bytes);
    ...
    let cas_sha256 = knowledge::sha256_hex(&cas_bytes);
...
async fn extraction_retry_uses_queue_redelivery_for_the_same_business_revision() {
    ...
    let target = bidding::bidding::load_extraction(&pool, target_id, 1)
        .await
        .unwrap()
        .expect("the current business revision must be loadable");
    assert_eq!(target.extraction_generation, 1);
    assert!(
        bidding::bidding::load_extraction(&pool, target_id, 2)
            .await
            .unwrap()
            .is_none(),
    ...
    assert!(
        bidding::bidding::fail_extraction(
            &pool,
            target_id,
            1,
            "TEST_RETRYABLE",
    ...
    assert!(
        bidding::bidding::load_extraction(&pool, target_id, 1)
            .await
            .unwrap()
            .is_some(),
...
async fn confirmed_clause_cannot_patch_kind_before_leaving_old_membership() {
    ...
    .unwrap();
    let forbidden = bidding::bidding::mutate_clause(
        &pool,
        seed.project_id,
        clause_id,
        "patch",
    ...
    let unchanged = bidding::bidding::list_clauses(&pool, seed.project_id, false)
        .await
        .unwrap()
        .into_iter()
    ...
    .unwrap();
    let reconfirmed = bidding::bidding::mutate_clause(
        &pool,
        seed.project_id,
        clause_id,
        "confirm",
...
async fn converted_images_transfer_staging_to_source_artifact_owners() {
    ...
    let original_digest = knowledge::sha256_hex(original);
    let original_ref = platform::object_ref(&original_digest);
    ...
    bidding::bidding::upload_document(
        &pool,
        original_staging_id,
        UploadDocument {
            id: document_id,
    ...
    let conversion = bidding::bidding::load_document_conversion(&pool, document_id, 1)
        .await
        .unwrap()
        .expect("the current conversion revision must be loadable");
    assert_eq!(conversion.conversion_generation, 1);
    assert!(
        bidding::bidding::load_document_conversion(&pool, document_id, 2)
            .await
            .unwrap()
            .is_none(),
    ...
    let image = b"\x89PNG\r\n\x1a\nconverted-image";
    let image_digest = knowledge::sha256_hex(image);
    ...
    let successor = bidding::bidding::document_conversion_successor(&pool, document_id, 1)
        .await
        .unwrap()
