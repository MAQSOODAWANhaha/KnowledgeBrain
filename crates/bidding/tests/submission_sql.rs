...
fn request_identity(payload: &Value) -> (Vec<u8>, String) {
    ...
    let sha256 = knowledge::sha256_hex(&bytes);
    (bytes, sha256)
}
...
impl TestBlobs {
    fn persist(entries: &[(&str, &[u8])]) -> Self {
        std::fs::create_dir_all(platform::object_dir()).expect("create test object directory");
        for (digest, bytes) in entries {
            std::fs::write(platform::blob_path(digest), bytes).expect("persist test object bytes");
        }
        Self(
            entries
                .iter()
...
impl Drop for TestBlobs {
    fn drop(&mut self) {
        for digest in &self.0 {
            let _ = std::fs::remove_file(platform::blob_path(digest));
        }
    }
}
...
async fn upload_pending_pdf_attachment(pool: &PgPool, seed: &SubmissionSeed) -> (Uuid, Uuid) {
    ...
    let digest = knowledge::sha256_hex(&original);
...
async fn stage_render_page(pool: &PgPool, page_ordinal: usize) -> (Uuid, Value) {
    ...
    let digest = knowledge::sha256_hex(&page);
    let object_ref = format!("objects/{digest}");
    let staging_id = stage_object(
        pool,
        &object_ref,
...
async fn schedule_and_load_render(
    ...
    seed: &SubmissionSeed,
    manifest: &Value,
) -> (Uuid, bidding::bid_submission::SubmissionRenderTarget) {
    ...
    let target = bidding::bid_submission::load_submission_render(pool, render_job_id, 1)
        .await
...
async fn procedural_text_change_terminals_previous_classification_and_decision() {
    ...
    let create_context = bidding::bidding::MutationContext::new(
        seed.actor.clone(),
    ...
    .await
    .unwrap();
    let confirm_context = bidding::bidding::MutationContext::new(
        seed.actor.clone(),
    ...
    .await
    .unwrap();
    let resolve_context = bidding::bidding::MutationContext::new(
        seed.actor.clone(),
    ...
    let patch_context = bidding::bidding::MutationContext::new(
        seed.actor.clone(),
...
async fn upload_shot_artifact(pool: &PgPool, seed: &SubmissionSeed, bytes: &[u8]) -> String {
    ...
    let digest = knowledge::sha256_hex(bytes);
    let object_ref = format!("objects/{digest}");
    let request = json!({
...
async fn rejected_upload_is_domain_zero_write_and_platform_abandon_is_tracked() {
    ...
    assert!(
        platform::abandon_object_upload(&pool, staging_id, &seed.actor)
            .await
            .unwrap(),
...
async fn publishing_a_manifest_rejects_when_current_identity_changed() {
    ...
    let output_sha256 = knowledge::sha256_hex(&output_bytes);
...
async fn submission_render_job_is_idempotent_revision_fenced_and_terminally_observable() {
    ...
    let first = bidding::bid_submission::schedule_submission_render(
        &pool,
        render_job_id,
        seed.project_id,
        manifest_id,
    ...
    let replay = bidding::bid_submission::schedule_submission_render(
        &pool,
        replay_candidate,
        seed.project_id,
        manifest_id,
    ...
    assert!(
        bidding::bid_submission::load_submission_render(&pool, render_job_id, 2)
            .await
            .unwrap()
            .is_none(),
    ...
    assert_eq!(
        bidding::bid_submission::fail_submission_render(
            &pool,
            render_job_id,
            2,
    ...
    assert!(
        bidding::bid_submission::load_submission_render(&pool, render_job_id, 1)
            .await
            .unwrap()
            .is_some(),
    ...
    .await;
    let stale_revision = bidding::bid_submission::publish_submission_output(
        &pool,
        bidding::bid_submission::PublishSubmissionOutput {
            staging_id,
            id: output_id,
            render_job_id,
            target_revision: 2,
    ...
    let published = bidding::bid_submission::publish_submission_output(
        &pool,
        bidding::bid_submission::PublishSubmissionOutput {
            staging_id,
            id: output_id,
    ...
    let completed =
        bidding::bid_submission::get_submission_render_job(&pool, seed.project_id, render_job_id)
            .await
            .unwrap()
    ...
    let failed =
        bidding::bid_submission::get_submission_render_job(&pool, seed.project_id, failed_job_id)
            .await
            .unwrap()
    ...
    assert!(
        bidding::bid_submission::get_submission_render_job(&pool, Uuid::new_v4(), failed_job_id)
            .await
            .unwrap()
            .is_none(),
...
async fn docx_without_eligible_quote_freezes_fixed_placeholder() {
    ...
    assert_eq!(
        ...
        knowledge::sha256_hex("> [报价尚未最终确认]".as_bytes())
    );
}
...
async fn procedural_router_and_template_promotions_are_cas_fenced_and_durable() {
    ...
    let segment_sha256 = knowledge::sha256_hex(segment_bytes);
    let stable_key = knowledge::sha256_hex(format!("{clause_id}:{segment_sha256}").as_bytes());
    sqlx::query(
    ...
    for (version, override_value) in &procedural_versions {
        ...
        let content_sha256 = knowledge::sha256_hex(&canonical_payload);
    ...
    let template_sha256 = knowledge::sha256_hex(&template_payload);
...
async fn pdf_attachment_preparation_requires_a_contiguous_frozen_page_set() {
    ...
    assert!(
        bidding::bid_submission::load_attachment_preparation(&pool, preparation_job_id, 2)
            .await
            .unwrap()
            .is_none(),
        "a stale attachment revision must not load the target"
    );
    let target = bidding::bid_submission::load_attachment_preparation(&pool, preparation_job_id, 1)
        .await
        .unwrap()
    ...
    let result = bidding::bid_submission::publish_attachment_preparation(
        &pool,
        preparation_job_id,
        1,
        &invalid_pages,
    ...
    assert_eq!(
        bidding::bid_submission::fail_attachment_preparation(
            &pool,
            preparation_job_id,
            1,
            "INVALID_PAGE_SET",
    ...
    assert!(
        platform::abandon_object_upload(&pool, page_staging, worker_actor)
            .await
            .unwrap()
    );
    ...
    let unavailable_digest = knowledge::sha256_hex(&unavailable_page);
    let page_one = json!({
    ...
    let partial_publish = bidding::bid_submission::publish_attachment_preparation(
        &pool,
        preparation_job_id,
        1,
    ...
    assert!(
        platform::abandon_object_upload(&pool, page_zero_staging, worker_actor)
            .await
            .unwrap()
    );
}
...
async fn attachment_preparation_retries_are_owned_by_oxana_and_do_not_exhaust_in_db() {
    ...
    for retry in 1..=6 {
        let target =
            bidding::bid_submission::load_attachment_preparation(&pool, preparation_job_id, 1)
                .await
                .unwrap()
...
async fn rejecting_or_deleting_attachment_cancels_and_fences_pending_preparation() {
    ...
    for (action, expected_error_code) in [
        ...
        let stale = bidding::bid_submission::publish_attachment_preparation(
            &pool,
            preparation_job_id,
            1,
            &json!([]),
...
async fn attachment_validation_rejects_unavailable_frozen_object_identity() {
    ...
    let digest = knowledge::sha256_hex(&attachment_bytes);
    let object_ref = format!("objects/{digest}");
    let upload = json!({
        "attachment_id": attachment_id,
        "project_id": seed.project_id,
...
async fn confirmed_pdf_attachment_is_frozen_into_manifest_and_rendered_from_pages() {
    ...
    let original_pdf = bidding::render_manifest_document(
        bidding::submission::GateFormat::Pdf,
        "保证金附件原件",
    ...
    let original_digest = knowledge::sha256_hex(&original_pdf);
    let page_digests = pages
        .iter()
        .map(|page| knowledge::sha256_hex(page))
        .collect::<Vec<_>>();
    ...
    let prepared = bidding::bid_submission::publish_attachment_preparation(
        &pool,
        preparation_job_id,
        1,
        &json!(render_pages),
    ...
    let manifest_context = bidding::bidding::MutationContext::new(
        seed.actor.clone(),
        manifest_key,
    ...
    let input = bidding::bid_submission::manifest_render_input(&pool, seed.project_id, manifest_id)
        .await
        .unwrap();
    ...
    for row in input["assets"].as_array().unwrap() {
        ...
        let stored = bidding::bid_submission::read_manifest_render_asset(
            &pool,
            seed.project_id,
            manifest_id,
            asset_id,
        ...
        let locator = match stored.source_kind.as_str() {
            "procedural_attachment" => {
                bidding::ManifestRenderAssetLocator::ProceduralAttachmentOriginal {
            ...
            "procedural_attachment_page" => {
                ...
                bidding::ManifestRenderAssetLocator::ProceduralAttachmentPage {
                    part_key: stored.source_locator["part_key"]
                        .as_str()
                        .unwrap()
                        .to_string(),
        ...
        assets.push(bidding::ManifestRenderAsset {
            manifest_ordinal,
            locator,
    ...
    let docx = bidding::render_manifest_document(
        bidding::submission::GateFormat::Docx,
        "投标文件",
        &parts,
        &assets,
    )
    ...
    let pdf = bidding::render_manifest_document(
        bidding::submission::GateFormat::Pdf,
        "投标文件",
        &parts,
        &assets,
    ...
    std::fs::write(
        platform::blob_path(&page_digests[0]),
        unique_png(Uuid::new_v4()),
    )
    .unwrap();
    let corrupt = bidding::bid_submission::read_manifest_render_asset(
        &pool,
        seed.project_id,
        manifest_id,
        page_asset_ids
    ...
    std::fs::write(platform::blob_path(&page_digests[0]), &pages[0]).unwrap();
    ...
    let missing_context = bidding::bidding::MutationContext::new(
        seed.actor.clone(),
        missing_key.clone(),
        &json!({"manifest_id":missing_manifest_id,"project_id":seed.project_id,"format":"docx"}),
    )
    .unwrap();
    let missing = bidding::bid_submission::create_submission_manifest(
        &pool,
        missing_manifest_id,
        seed.project_id,
        "docx",
...
async fn manifest_creation_rolls_back_for_missing_and_corrupt_physical_blob() {
    ...
    .unwrap();
    let missing_error = bidding::bid_submission::create_submission_manifest(
        &pool,
        missing_manifest_id,
        seed.project_id,
        "docx",
    ...
    std::fs::create_dir_all(platform::object_dir()).unwrap();
    std::fs::write(platform::blob_path(digest), unique_png(Uuid::new_v4())).unwrap();
    let corrupt_manifest_id = Uuid::new_v4();
    let corrupt_key = format!("corrupt-blob-{corrupt_manifest_id}");
    let corrupt_context = bidding::bidding::MutationContext::new(
        seed.actor.clone(),
        corrupt_key.clone(),
        &json!({"manifest_id":corrupt_manifest_id,"project_id":seed.project_id,"format":"docx"}),
    )
    .unwrap();
    let corrupt_error = bidding::bid_submission::create_submission_manifest(
        &pool,
        corrupt_manifest_id,
        seed.project_id,
        "docx",
