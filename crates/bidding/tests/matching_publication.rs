use bidding::matching::run_match_route_v1;
use serde_json::json;
use sqlx::{PgPool, Row};
use bidding::bid_matching::{
...
async fn matching_schedule_binding_freezes_target_identity_for_the_idempotency_key() {
    ...
    let first_context =
        bidding::bidding::MutationContext::new(actor.clone(), &first_key, &first_payload).unwrap();
    let first = bidding::bid_matching::bind_schedule_target(&pool, project_id, &first_context)
        .await
        .unwrap()
    ...
    .await
    .unwrap();
    let replay = bidding::bid_matching::bind_schedule_target(&pool, project_id, &first_context)
        .await
        .unwrap()
        .expect("the completed receipt keeps its target");
    assert_eq!(replay, first);
    let next_context = bidding::bidding::MutationContext::new(
        actor.clone(),
        format!("bind-next-{project_id}"),
        &first_payload,
    )
    .unwrap();
    let next = bidding::bid_matching::bind_schedule_target(&pool, project_id, &next_context)
        .await
        .unwrap()
    ...
    let empty_context =
        bidding::bidding::MutationContext::new(actor.clone(), &empty_key, &empty_payload).unwrap();
    assert!(
        bidding::bid_matching::bind_schedule_target(
            &pool,
    ...
    let empty_next_context = bidding::bidding::MutationContext::new(
        actor.clone(),
    ...
    let mismatched_project_context =
        bidding::bidding::MutationContext::new(actor.clone(), &first_key, &empty_payload).unwrap();
    ...
    for (invalid_actor, expected) in [
        ...
        .unwrap();
        assert_database_error(
            bidding::bid_matching::bind_schedule_target(&pool, project_id, &context)
                .await
                .unwrap_err(),
            expected,
        );
...
fn matching_schedule_payload_without_eligible_scope(
    ...
    let requirement_sha256 = knowledge::sha256_hex(requirement.as_bytes());
    let requirement_set_sha256 = knowledge::sha256_hex(
    ...
    json!({
        ...
        "eligible_scope_sha256": knowledge::sha256_hex(b"[]"),
        "routes": [
            {
                ...
                "route_scope_sha256": knowledge::sha256_hex(b"technical:00000000-0000-0000-0000-000000000000")
            },
            {
                ...
                "ordinal": 1,
                "empty_policy": "clear_route",
                "route_scope_sha256": knowledge::sha256_hex(b"commercial")
            }
        ],
        "requirements": [{
            "id": Uuid::new_v4(),
...
async fn matching_schedule_payload_with_complete_product_scope(
    ...
    let mut frozen_requirements = requirements
        ...
        .map(|(clause_id, text)| {
            (
                ...
                *clause_id,
                (*text).to_string(),
                knowledge::sha256_hex(text.as_bytes()),
            )
        })
        .collect::<Vec<_>>();
    ...
    let frozen_products = eligible_rows
        ...
        .map(|row| {
            ...
            let identity_sha256 = knowledge::sha256_hex(
        ...
        .collect::<Vec<_>>();
    let eligible_scope_sha256 = knowledge::sha256_hex(
        &serde_json::to_vec(
            &frozen_products
                .iter()
        ...
        "routes": [
            {
                ...
                "ordinal": 0,
                "empty_policy": "clear_route",
                "route_scope_sha256": knowledge::sha256_hex(
...
async fn assert_matching_schedule_rejected(
    ...
    let request_sha256 = knowledge::sha256_hex(&request_bytes);
...
async fn schedule_current_delivery(
    ...
    context: &bidding::bid_matching::ScheduleMutationContext,
) -> bidding::bid_matching::ScheduleReceipt {
    ensure_matching_schedule_intent_fixture(pool, project_id).await;
    ...
    if !scheduled {
        assert!(
            bidding::bid_matching::execute_schedule(
                pool,
                intent_id,
...
async fn empty_matching_scope_publishes_one_manifest_and_zero_jobs() {
    ...
    let context = bidding::bid_matching::ScheduleMutationContext::system();
    let scheduled =
...
async fn commercial_review_and_reject_project_frozen_explanatory_evidence() {
    ...
    let scheduled = schedule_current_delivery(
        ...
        &bidding::bid_matching::ScheduleMutationContext::system(),
    )
    .await;
    ...
    for (artifact_id, ordinal) in [
        ...
        .bind(knowledge::sha256_hex(artifact_id.as_bytes()))
        .execute(&mut *fixture)
        .await
        .unwrap();
        sqlx::query(
    ...
    .bind(knowledge::sha256_hex(canonical_payload))
    .execute(&mut *fixture)
    .await
    .unwrap();
    ...
    ) in candidates
    {
        ...
        let chunk_sha256 = knowledge::sha256_hex(quote.as_bytes());
        let candidate_identity_sha256 = knowledge::sha256_hex(candidate_id.as_bytes());
        let evidence_v1_sha256 = knowledge::sha256_hex(evidence_artifact_id.as_bytes());
        sqlx::query(
    ...
    let regenerate_request_sha256 = knowledge::sha256_hex(&regenerate_request_bytes);
...
async fn matching_schedule_preserves_sixty_five_eligible_versions_with_zero_hits() {
    ...
    let document_digest = knowledge::sha256_hex(document_bytes);
...
async fn matching_publication_freezes_evidence_and_builds_unsectioned_pick_sets() {
    ...
    let digest = knowledge::sha256_hex(file_bytes);
    ...
    let source_digest = knowledge::sha256_hex(source_bytes);
    ...
    let source_span_digest = knowledge::sha256_hex(&source_span_bytes);
    sqlx::query(
    ...
    let schedule_context = bidding::bid_matching::ScheduleMutationContext::system();
    let scheduled = schedule_current_delivery(
        &pool,
        project_id,
        ScheduleEnvironment {
    ...
    let commercial_job = scheduled
        ...
        .unwrap();
    assert!(
        bidding::bid_matching::start_matching_execution(
            &pool,
            commercial_job.id,
    ...
    let commercial_request = bidding::bid_matching::start_matching_execution(
        &pool,
        commercial_job.id,
    ...
    assert!(
        !bidding::bid_matching::fail_matching(
            &pool,
            &stale_request,
            "STALE_TARGET",
    ...
    let ordinary_job = scheduled
        ...
        .unwrap();
    let staging_request = bidding::bid_matching::start_matching_execution(
        &pool,
        ordinary_job.id,
    ...
    let staged_source = StagedSourceArtifactV1 {
        ...
        chunk_utf8: "x".into(),
        chunk_sha256: knowledge::sha256_hex(b"x"),
        chunk_byte_length: 1,
        retrieval_rank: 1,
    ...
    assert!(
        bidding::bid_matching::publish_route(&pool, &staging_request, duplicate_source_report,)
            .await
            .is_err(),
    ...
    let retried_staging_request = bidding::bid_matching::start_matching_execution(
        &pool,
        staging_request.job_id,
    ...
    let unsectioned_job = scheduled
        ...
        .unwrap();
    let commit_request = bidding::bid_matching::start_matching_execution(
        &pool,
        unsectioned_job.id,
    ...
    let invalid_report_sha256 = knowledge::sha256_hex(&invalid_commit_report.canonical_payload);
    assert!(
        bidding::bid_matching::publish_route(&pool, &commit_request, invalid_commit_report)
            .await
            .is_err(),
    ...
    let routes = bidding::bid_matching::current_routes(&pool, project_id)
        .await
        .unwrap();
    ...
    let report = bidding::bid_matching::current_route_report(&pool, project_id, route_id)
        .await
        .unwrap()
        .unwrap();
    let historical_report = bidding::bid_matching::matching_report_artifact_json(
        &pool,
        project_id,
    ...
    assert_eq!(
        knowledge::sha256_hex(canonical_payload.as_bytes()),
    ...
    let candidates =
        bidding::bid_matching::current_route_supported_candidates(&pool, project_id, route_id)
            .await
            .unwrap();
    assert!(
        candidates.len() >= 2,
    ...
    let forged_sha256 = knowledge::sha256_hex(&forged_bytes);
    ...
    let noncanonical_sha256 = knowledge::sha256_hex(&noncanonical_bytes);
    ...
    let context =
        bidding::bidding::MutationContext::new(actor.clone(), format!("pick-{project_id}"), &body)
            .unwrap();
    let receipt = bidding::bid_matching::replace_route_pick_set(
        &pool,
    ...
    let replay = bidding::bid_matching::replace_route_pick_set(
        &pool,
        ReplaceRoutePickSetV1 {
            project_id,
            route_id,
    ...
    .await
    .unwrap();
    assert_eq!(knowledge::sha256_hex(&route_payload), route_digest);
    ...
    let ordinary_report =
        bidding::bid_matching::current_route_report(&pool, project_id, ordinary_route_id)
            .await
            .unwrap()
            .unwrap();
    let ordinary_candidates = bidding::bid_matching::current_route_supported_candidates(
        &pool,
        project_id,
        ordinary_route_id,
    )
    ...
    let ordinary_context = bidding::bidding::MutationContext::new(
        actor.clone(),
        format!("pick-ordinary-{project_id}"),
        &ordinary_body,
    )
    .unwrap();
    let ordinary_receipt = bidding::bid_matching::replace_route_pick_set(
        &pool,
        ReplaceRoutePickSetV1 {
            project_id,
    ...
    let mutation_context = bidding::bidding::MutationContext::new(
        actor.clone(),
    ...
    sqlx::query("UPDATE bid_projects SET status='ended',ended_at=clock_timestamp() WHERE id=$1")
        ...
        .await
        .unwrap();
    let historical = bidding::bid_matching::matching_report_artifact_json(
        &pool,
        project_id,
        report.get("report_id"),
    )
