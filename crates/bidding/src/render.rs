...
fn validate_frozen_asset_identity(asset: &ManifestRenderAsset) -> Result<(), String> {
    ...
    let actual_digest = knowledge::sha256_hex(&asset.bytes);
...
fn cjk_font_bytes() -> Result<&'static [u8], String> {
    let digest = knowledge::sha256_hex(PDF_FONT_BYTES);
...
mod tests {
    ...
    fn rejects_wrong_object_ref_non_image_mime_and_damaged_image() {
        ...
        damaged.digest = knowledge::sha256_hex(&damaged.bytes);
    ...
    fn frozen_pdf_font_matches_declared_identity() {
        ...
        assert_eq!(knowledge::sha256_hex(PDF_FONT_BYTES), PDF_FONT_SHA256);
        ...
        assert!(FONT_MANIFEST.contains(&knowledge::sha256_hex(FONT_LICENSE)));
    }
