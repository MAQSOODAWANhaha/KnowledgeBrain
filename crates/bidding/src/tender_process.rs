...
pub struct ConvertedTenderSourceRevision {
    ...
    pub source_unit_set_sha256: String,
}
    ...
    async fn enrich(
        ...
        Ok(VisionEnrichment {
            ...
            operation_contract: FrozenContractIdentity {
                id: stable_uuid(
                    b"tender-vision-operation",
                    TENDER_VISION_OPERATION.as_bytes(),
                ),
    ...
    T: TenderProcessTransport,
{
    ...
    pub async fn process(
        ...
        let converted_source_id = stable_uuid(
            ...
            format!(
                ...
                document.converter_contract_id,
    ...
    async fn build_publication(
        ...
        let source_unit_set_sha256 = sha256_hex(
            source_units
                .iter()
                .map(|unit| unit.content_sha256.as_str())
                .collect::<String>()
                .as_bytes(),
        );
...
impl TenderDocumentProcessRepository for PgTenderDocumentProcessRepository {
    async fn load_frozen_document(
        ...
        let row = crate::bid_authoring_v2::load_tender_document_process_input_v2(
            &self.pool,
    ...
    async fn stage_object(
        ...
        if bytes.is_empty()
            || bytes.len() > MAX_TENDER_IMAGE_BYTES.max(MAX_TENDER_SOURCE_TEXT_BYTES)
        {
    ...
    async fn publish(
        ...
        let source = json!({
            ...
            "source_unit_set_sha256": publication.converted_source.source_unit_set_sha256,
        });
...
fn verify_frozen_document(
    ...
    if document.role_revision_sha256.len() != 64 || document.converter_contract_sha256.len() != 64 {
        return Err(TenderDocumentProcessError::FrozenInputMismatch(
...
fn map_non_image_kind(
    ...
    match kind {
        ...
        StructuredSourceUnitKind::AttachmentRegion => Ok(PublishedSourceUnitKind::AttachmentRegion),
        StructuredSourceUnitKind::ImageRegion => Err(TenderDocumentProcessError::StructuredSource(
            "unprocessed image unit".into(),
        )),
    }
}
