...
{
    let Some(request) =
        crate::bid_matching::start_matching_execution(pool, job_id, target_revision)
            .await
...
async fn publish_report(
    ...
    let receipt = match crate::bid_matching::publish_route(pool, request, transport).await {
        Ok(receipt) => receipt,
        Err(error) => {
...
async fn settle_retryable(
    ...
    let updated = crate::bid_matching::fail_matching(pool, request, code, detail, true)
        .await
...
async fn settle(
    ...
    let updated = crate::bid_matching::fail_matching(pool, request, code, detail, retryable)
        .await
        .map_err(|error| error.to_string())?;
    if retryable {
        retryable_settlement_result(updated, code, detail)
