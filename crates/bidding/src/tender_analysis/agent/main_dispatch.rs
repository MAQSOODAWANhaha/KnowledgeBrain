...
fn blocked(
    ...
    if context::scope_is_blocked(state, &projection.source_scope)?
        && repair_recovery::history_query(state, &projection.source_scope)?.is_some()
    {
        // Existing, not-yet-delivered repair history permits bounded recovery
        // navigation. It grants neither writing permission nor a fresh root.
        return Ok(false);
    }
    blocked_scope(input, state, limits, &projection.source_scope)
}
...
fn install(
    ...
    assigned: Option<Projection>,
) -> Result<(), String> {
    let previous = state.dispatch.active.clone();
    let Some(assigned) = assigned else {
        state.dispatch.active = None;
        state.repair.tasks.active = None;
        return Ok(());
    };
    let work = work(input, &config.limits, state, &assigned);
    let watch = match &assigned.owner {
        Active::Repair(id) => {
            repair::tasks::select(state, id, &config.limits)?;
            repair::tasks::active(state)
                .ok_or("repair dispatch missing")?
                .1
                .watch
                .clone()
        }
        Active::Ordinary(id) => {
            ...
            let entry = state
                ...
                .entries
                .get_mut(id)
                .ok_or("dispatch root missing")?;
            if entry.watch.recovery == Recovery::Blocked
                && !entry
                    .attempted_dependencies
                    .contains(&assigned.dependencies_sha256)
            {
                entry.watch.no_progress_turns = 0;
                entry.watch.focus_turns = 0;
                entry.watch.recovery = Recovery::Replan;
            }
            entry
                .attempted_dependencies
                .insert(assigned.dependencies_sha256.clone());
            entry.watch.clone()
        }
    };
    state.dispatch.active = Some(assigned.owner);
    state.main_progress.watch = watch;
    state.main_work = Some(work);
    if previous != state.dispatch.active
...
pub(in crate::tender_analysis) fn confirm(
    ...
    let packet: Value = serde_json::from_str(
        body["messages"]
            ...
            .ok_or("dispatch request packet missing")?,
    )
    .map_err(|e| e.to_string())?;
    if packet["main_dispatch"].is_null() {
        return match continuation(input, config, state)? {
            Continuation::EnterReview => Ok(()),
            _ => Err(
                "dispatch identity missing: invalid type: null, expected struct Projection".into(),
            ),
        };
    }
    let sent: Projection = serde_json::from_value(packet["main_dispatch"].clone())
        .map_err(|e| format!("dispatch identity missing: {e}"))?;
    if projection(input, config, state)?.as_ref() != Some(&sent) {
        return Err("reserved Main dispatch identity changed".into());
    }
    if state.dispatch.entries.is_empty() {
        state.dispatch.entries = seeds(input, &config.limits)?;
    }
    install(input, config, state, Some(sent))
}

pub(in crate::tender_analysis) fn check_ready(
    input: &FrozenInput,
    config: &Config,
    state: &Checkpoint,
) -> Result<(), AgentError> {
    ...
    match continuation(input, config, state).map_err(invalid)? {
        Continuation::EnterReview => return Ok(()),
        Continuation::Stop => {
            return Err(error(
                "AGENT_TURN_BUDGET_EXCEEDED",
                "no_executable_main_tasks: all remaining roots blocked or exhausted; checkpoint and costs retained",
            ));
        }
        Continuation::Continue(_) => {}
    }
    let assigned=projection(input,config,state).map_err(invalid)?
        .ok_or_else(||error("AGENT_TURN_BUDGET_EXCEEDED", "no_executable_main_tasks: all remaining roots blocked or exhausted; checkpoint and costs retained"))?;
    if blocked(input, state, &config.limits, &assigned).map_err(invalid)? {
        let global = match &assigned.owner {
            Active::Ordinary(id) => state
                .dispatch
                .entries
                .get(id)
                .is_some_and(|entry| entry.source_id.is_none()),
            Active::Repair(_) => false,
        };
        if !global {
            return Err(invalid("assigned Main task dependencies are blocked"));
        }
    }
    if let Active::Repair(id) = &assigned.owner
        && repair::tasks::current(state, &config.limits)
            .map_err(invalid)?
            .iter()
            .find(|task| &task.id == id)
            .is_none_or(|task| task.exhausted)
    {
        return Err(error(
            "AGENT_TURN_BUDGET_EXCEEDED",
            "assigned repair allowance exhausted; existing ledger retained",
        ));
    }
    if let Active::Ordinary(id) = &assigned.owner
        && let Some(entry) = state.dispatch.entries.get(id)
        && exhausted(entry, &assigned.dependencies_sha256, &config.limits).map_err(invalid)?
        && main_handoff::check(input, config, state).is_err()
    {
        return Err(error(
            "AGENT_TURN_BUDGET_EXCEEDED",
            "Main root allowance exhausted; cumulative costs retained",
        ));
    }
    Ok(())
}
...
pub(in crate::tender_analysis) fn after_batch(
    ...
    config: &Config,
    state: &mut Checkpoint,
    charged: Option<&Active>,
    batch_failed: bool,
    analysis_mutated: bool,
) -> Result<(), String> {
    let turn = state.turn + 1;
    if let Some(charged) = charged {
        if state.dispatch.active.as_ref() != Some(charged) {
            return Err("charged Main owner differs from received assignment".into());
        }
        if state
            .dispatch
            .last_committed_turn
            .is_some_and(|last| last >= turn)
        {
            return Err("dispatch batch already charged".into());
        }
        match charged {
            Active::Repair(id) => repair::tasks::after_batch(
                state,
                id,
                turn,
            ...
            Active::Ordinary(id) => {
                let projected = root_projection(
                    ...
                    state
                        ...
                        .entries
                        .get(id)
                        .ok_or("charged root missing")?,
                )?;
                let is_global = state.dispatch.entries[id].source_id.is_none();
                let scope = state
                    .main_work
                    .as_ref()
                    .map(|w| w.source_scope.clone())
                    .unwrap_or_else(|| projected.source_scope.clone());
                let complete = !batch_failed
                    && state.pending_coverage.is_none()
                    && !context::scope_is_blocked(state, &scope)?
                    && !repair_recovery::available(state, &scope)?
                    && source_complete(input, state, &scope)?;
                let cap = {
                    let entry = state
                        .dispatch
                        .entries
                        .get(id)
                        .ok_or("charged root missing")?;
                    if config.limits.pack_max_turns > 0 && entry.source_id.is_some() {
                        config.limits.pack_max_turns
                    } else {
                        repair::tasks::limit(&config.limits)?
                    }
                };
                let entry = state
                    .dispatch
                    ...
                    .get_mut(id)
                    .ok_or("charged root missing")?;
                entry.spent_batches = entry
                    .spent_batches
                    .checked_add(1)
                    .ok_or("dispatch charge overflow")?;
                entry.spent_replans = entry.spent_replans.max(state.main_progress.watch.replans);
                entry.watch = state.main_progress.watch.clone();
                entry
                    .attempted_dependencies
                    .insert(projected.dependencies_sha256.clone());
                if entry.spent_batches >= cap {
                    entry.watch.recovery = Recovery::Blocked;
                }
                state.main_progress.watch = entry.watch.clone();
                entry.completed_dependencies =
                    (complete && !is_global).then_some(projected.dependencies_sha256);
                if complete {
                    state
                        .main_progress
                        .blockers
                        .retain(|blocker| !blocker.scope.iter().all(|id| scope.contains(id)));
                }
            }
        }
        state.dispatch.last_committed_turn = Some(turn);
    }
    if state.role != Role::Main || state.done {
        state.dispatch.active = None;
        state.repair.tasks.active = None;
    ...
    repair::tasks::sync(state, &config.limits)?;
    if !batch_failed
        && !analysis_mutated
        && state.pending_coverage.is_none()
        && main_handoff::check(input, config, state).is_ok()
    {
        if let Some(Active::Ordinary(id)) = &state.dispatch.active {
            let scope = rule_contract::scope_sha256(input, &state.analysis)?;
            if let Some(entry) = state.dispatch.entries.get_mut(id)
                && entry.source_id.is_none()
            {
                entry.completed_dependencies = Some(scope);
            }
        }
    ...
    if !batch_failed
        && !analysis_mutated
        && matches!(
            continuation(input, config, state)?,
            Continuation::EnterReview
        )
        && (state.pending_coverage.is_none() || leftover_incomplete(input, state)?)
    {
        main_handoff::enter_with_closed_omissions(input, config, state)?;
        return Ok(());
    }
    // Pending reads belong to the current owner until actually delivered.
    if state.pending_coverage.is_some()
        || state
            .main_work
            .as_ref()
            .is_some_and(|work| !work.deferred_sources.is_empty())
        || (batch_failed && check_ready(input, config, state).is_ok())
    {
        return Ok(());
    }
    let next = next(input, config, state)?;
    install(input, config, state, next)
}
