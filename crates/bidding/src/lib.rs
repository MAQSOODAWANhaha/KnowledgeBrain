...
fn clause_unit_id(clause: &crate::bidding::Clause) -> Uuid {
    clause
...
pub async fn list_match_units(
    ...
    let routes = crate::bid_submission::current_routes(pool, project_id)
        .await
        .map_err(|e| e.to_string())?;
    let clauses = crate::bidding::list_clauses(pool, project_id, false)
        .await
        .map_err(|e| e.to_string())?;
...
pub fn matching_schedule_environment() -> crate::bid_matching::ScheduleEnvironment {
    ...
    .to_string();
    crate::bid_matching::ScheduleEnvironment { environment }
}
