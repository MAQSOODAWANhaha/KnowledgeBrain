            ...
            || state.read_bytes >= l.max_read_bytes
        {
            ...
            match serde_json::from_str::<Value>(&call.arguments).map_err(|e|e.to_string()).and_then(|args|crate::tender_analysis::evidence_refs::expand(input,&args)){
                ...
                Ok(args)=>{
                    ...
                    let output=agent_work::check_read_scope(input,state,&call.name,&args).and_then(|()|next.invoke(input,result,&call.name,&args,super::tools::ToolLimits { max_result_bytes:l.max_tool_result_bytes,max_docx_bytes:l.max_docx_bytes,max_review_rounds:l.max_review_rounds },));
                    match output {
                        Ok(value)=>{
                            ...
                            else{state.read_bytes+=bytes;state.workspace=next;if let Some((id,_))=image{images.push(id.to_owned());}Ok(value)}
                        }
                        Err(e)=>Err(e),
                    }
                }
...
pub(super) async fn request(
    ...
    if state.execution().watch.needs_replan_context() {
        while agent_work::evict_history(state, false) {}
    }
    loop {
        let mut messages = vec![
            json!({"role":"system","content":if state.workspace.reviewing{REVIEWER}else{MAIN}}),
            json!({"role":"user","content":json!({"analysis_sha256":state.workspace.draft.analysis_sha256,"draft_sha256":digest(&state.workspace.draft).map_err(invalid)?,"turn":state.turn,"local_work":agent_work::packet(state)}).to_string()}),
        ];
        ...
        if agent_work::evict_history(state, true) {
            continue;
        } else if let Some(id) =
            crate::tender_analysis::views::defer_last_image(&mut state.transcript)
                .map_err(invalid)?
        {
            let coverage = if state.workspace.reviewing {
                &mut state.workspace.review_coverage
            } else {
                &mut state.workspace.source_coverage
            };
            coverage.views.remove(&id);
        } else {
            return Err(AgentError::new(
                "AGENT_TURN_BUDGET_EXCEEDED",
