...
async fn assert_seeded_fixture(pool: PgPool, fixture: Fixture) {
    let first_count =
        storage::knowledge_index_v2::rebuild_keyword_indexes_v2(&pool, fixture.version_id)
            .await
            .unwrap();
    ...
    let missing_error = storage::knowledge_index_v2::rebuild_keyword_indexes_v2(&pool, missing_id)
        .await
        .unwrap_err();
    ...
    let mut second_rebuild = tokio::spawn(async move {
        storage::knowledge_index_v2::rebuild_keyword_indexes_v2(&second_pool, version_id)
            .await
            .unwrap()
    });
...
async fn keyword_tokenizer_rebuild_concurrency_and_acl_are_exact() {
    ...
    assert_eq!(index::KEYWORD_TOKENIZER_V2, TOKENIZER);
    assert_eq!(index::KEYWORD_TOKENIZER_VERSION_V2, TOKENIZER_VERSION);
    for (input, expected) in [
        ...
        let rust_stream = index::keyword_token_stream_v2(input);
