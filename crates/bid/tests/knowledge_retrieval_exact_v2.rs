...
fn rerank_revision() -> RerankRevisionV2 {
    RerankRevisionV2 {
        ...
        provider_model_revision_sha256: domain::sha256_hex(b"exact-v2 reranker model"),
        config_revision_sha256: domain::sha256_hex(b"exact-v2 reranker config"),
...
async fn insert_policy_raw(
    ...
    .bind(domain::sha256_hex(&payload))
    .bind(payload)
...
async fn insert_embedding_revision_raw(
    ...
    let revision_sha256 = domain::sha256_hex(&canonical_revision_payload);
    sqlx::query(
        "INSERT INTO embedding_revisions_v2(
             revision_sha256,canonical_revision_payload,schema_version,
             provider_protocol_version,provider_model_identifier,
...
async fn embedding_revision_registry_binding_sidecars_and_revocation_are_enforced() {
    ...
    let unknown_sha = domain::sha256_hex(Uuid::new_v4().as_bytes());
