...
members = [
  ...
  "crates/search", "crates/obs", "crates/storage", "crates/worker",
]
...
Workspace                         访问控制根
  ...
  │                 ├── tags[]    分类，多对多
  │                 └── Chunk / 向量 / 抽取
  └── Product (kind=library)      公司资料 / 通用文档
        └── ProductVersion        资料批次：现行 / 2026
              └── Document
                    └── tags[]
```
...
**workspaces**：`id, name, slug unique, retrieval_config jsonb, created_at, updated_at`  
无配额列。无 TOKEN / 计费字段。

**users**：`id, email unique, password_hash null, created_at, updated_at`
...
- `kind=product`：型号文档。
- `kind=library`：通用文档（公司资质、制度、模板）。同一 Workspace 可有多个 library。
- 创建 Workspace 时插入默认 library：`slug=library`，`name=公司资料`。删除该默认行不允许（可改名）。
...
唯一 `(product_id, label)`。新建默认 `indexing_strategy` 四开关全 `true`。

**documents**：对齐 brain `knowledges`。关键列：`id, product_version_id, type, title, parse_status, pending_subtasks_count, summary_status, enable_status, file_name, file_size, file_hash, object_key, process_overrides, error_message, processed_at, deleted_at`  
...
- 解析 / 分块 / Wiki / 图谱 / clone 对 `kind=product` 与 `kind=library` **同一套**，差别只在检索是否被 `include_library` 拼进来。
- 换证、年审 = 对 library 做 `version:clone`（批次快照）。旧批次 `archived` 后产品问答默认不再命中。
- TAG 可在上传时带、也可事后改。改 TAG 不触发 reparse。
...
| shared | 6 | `default`, `summary`, `multimodal`, `graph`, `question`（**同一 key**） | 与 dedicated 竞争，一条任务只跑一次 |
...
级联：删 Workspace → 入队删其下全部 ProductVersion（`kb:delete`）；删 Product 同理。有 `parse_status ∈ {pending, processing, finalizing}` 时先 cancel 再删。默认 library 产品不可删。

`version_id=current` 解析为该 Product（含 library）的 `current_version_id`；空则 400。写入仅允许 `status=active` 的版本。TAG 的 CRUD 不影响解析状态。

### 4.1 上传校验

1. 调用方覆盖该 product 所属 Workspace。
2. 版本 `active`。
...
JWT：HS256，密钥 `JWT_SECRET`，TTL 24h。claims：`sub=user_id`, `exp`。每请求查 `workspace_members`。`POST /auth/register` 建用户；加入 Workspace 由 admin 加成员。

| 角色 | 能力 |
|---|---|
| viewer | 读、检索 |
| contributor | 上传、reparse、clone |
| admin / owner | 成员与产品管理 |

API key：`scope_type=workspace|product`。bootstrap：`KNOWLEDGEBRAIN_BOOTSTRAP_KEY`。
...
9. `enable_status=enabled`，写 `processed_at`。若还有多模态或 text chunk：`parse_status` 保持 `processing`。若既无 text 也无多模态：直接 `completed`。
10. `EnableMultimodel && StoredImages>0`：设 Redis `multimodal:pending:{document_id}=N`，入队 N 条 `image:multimodal`。否则立刻 enqueue `knowledge:post_process`。

检索过滤 `enable_status=enabled`。`parse_status=finalizing` 的文档已可搜。
...
defer：成功或最后一次重试才 DECR pending；≤0 或 DECR 失败 fallback → enqueue `knowledge:post_process`。document 已 cancelled/deleting：drop 但仍计数。单图死信不 fail 父文档。
...
### 7.2 matching（按招标需求推荐产品）

输入是**一组需求条款**，输出是按条款覆盖率排序的产品候选。不是单条 query 扫一遍 current。
...

{
  ...
  "requirements": [
    ...
    { "id": "r2", "text": "具备 ISO9001",       "weight": 1.0, "must": false, "tag_ids": [], "use_library": true }
  ],
...
- 鉴权必须落到一个 Workspace（JWT 成员或 API key 的 workspace scope）。带 `product_id` → 400。
- `requirements` 必填，1–30 条。每条 `text` 非空。只传顶层 `query`、无 `requirements` 时，视为一条 `id=q0` 的需求（兼容）。
- 可选 `tender_text`：用 Workspace 内任意一个 active 产品 current 的 `summary_model_id` 把长文本拆成 `requirements[]`，然后走同一套打分。不落库、不当知识。投标平台也可自己抽条款，只传 `requirements`。
- `product_ids` 非空：只在这些产品里评。空 = 该 Workspace 全部 `kind=product`。硬顶 50 个产品。
- `version_scope=current`（默认）：每个产品只用 `current_version`（须 `active`）。无 current 的产品跳过。
- `version_scope=all_active`：每个产品的全部 `active` 版本都搜；该产品的 `matched_version_id` 取条款综合分最高的版本。
- 条款 `use_library=true` 或请求级 `include_library=true`：该条款额外在本 Workspace 各 library 的 `current_version` 上检索（资质/认证）。library hit 记在对应产品的该条款下，不单独成候选。
...
Hit：`id, content, score, match_type, chunk_type, document_id, document_title, product_id, product_kind, version_id, version_label, is_current, tag_ids, tag_slugs, start_at, end_at`。
...
投标组稿走 assembly `/search`。按招标条款荐产品走 matching `/search` 或 `/match`。`/answer` 只给已指定产品的问答机器人。
