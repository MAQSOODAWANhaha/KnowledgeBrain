...
class EPUBParser(BaseParser):
    ...
    def _extract_metadata(self, book) -> Dict[str, object]:
    ...
    def _strip_internal_links(soup: BeautifulSoup) -> None:
        ...
        for link in soup.find_all("a"):
            ...
            href = (
                href_raw[0] if isinstance(href_raw, list) else href_raw or ""
            )
    ...
    def _rewrite_image_sources(
        ...
        for img in soup.find_all("img"):
            src_raw = img.get("src")
            src = src_raw[0] if isinstance(src_raw, list) else src_raw or ""
            src = str(src).strip()
            if not src:
                continue
