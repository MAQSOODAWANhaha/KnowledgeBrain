...
def _metadata_value_bytes(value: object) -> bytes:
    """Normalize a gRPC metadata value to bytes.

    Metadata values are typed as ``str | bytes``; never call ``str.startswith``
    or ``bytes.startswith`` on the union.
    """
...
def _authorization_token(value: object) -> bytes:
    """Return the token bytes, stripping a leading ``Bearer `` prefix if present."""
    token = _metadata_value_bytes(value)
    if token[: len(_BEARER_PREFIX)].lower() == _BEARER_PREFIX:
        return token[len(_BEARER_PREFIX) :]
    return token
...
class AuthInterceptor(grpc.ServerInterceptor):
    ...
    def intercept_service(self, continuation, handler_call_details):
        ...
        for key, value in handler_call_details.invocation_metadata or ():
            if key in ("authorization", b"authorization"):
                raw_meta = value
                break
        token_bytes = _authorization_token(raw_meta)
