...
> Target V2 分阶段实施和最终验收只以 [`../tender-to-submission-v2.md`](../tender-to-submission-v2.md) 与 [`../frontend-authoring.md`](../frontend-authoring.md) 为准，目标契约见 [`../../docs/bidding/authoring.md`](../../../docs/bidding/authoring.md)。本文保留 V1 命令、表名和门禁仅供删除扫描与历史证据定位，不得用于判定 Target V2 完成。
...
本文验收的是 **V1 runtime / dispatch 切片**，不是产品目标。产品是否完成只看编制契约与 [`../tender-to-submission-v2.md`](../tender-to-submission-v2.md) / [`../frontend-authoring.md`](../frontend-authoring.md)。修改职责边界前必须重新评审。实施后仍分别报告 `implemented`、`locally verified`、`committed`、`pushed`、`deployed` 和 `runtime accepted`。

## 1. 成功标准

就本文覆盖的 V1 runtime 切片，最终完成时必须同时成立：
...
共享平台合同见 [`../platform/runtime-foundation.md`](../../../plans/platform/runtime-foundation.md)，队列合同见 [`../platform/queue-runtime.md`](../../../plans/platform/queue-runtime.md)。
...
只有六项都有证据、删除矩阵闭合、两个 required job 对同一候选通过，且 `phase_1d_runtime_complete=true`，才可以声明 **本文的 V1 runtime 切片完成**。这不等于招投标产品完成。产品完成只看编制契约与 V2 实施方案。本地 dirty 测试、历史 evidence 或单个绿色 job 都不能提前使用该结论。
