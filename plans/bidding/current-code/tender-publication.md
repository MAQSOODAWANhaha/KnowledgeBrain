...
> 产品目标变了：招标文件仍要上传、转换、定位来源，但不再以「确认条款 → 匹配向导」为主路径，而是驱动动态大纲与编制画布。SourceUnit / RequirementSet / Workspace 见编制契约，实施见 [`../tender-to-submission-v2.md`](../tender-to-submission-v2.md)。本文记录可复用的 SourceSpan/转换 seam，以及因目标变更而待替换的固定 clause/family 主流程。
