...
| 所有者 | Bidding |
| 队列依赖 | [`../platform/queue-runtime.md`](../../../plans/platform/queue-runtime.md) |

本文只定义招投标业务 target 如何调用 Oxana，**不是产品流程**。产品目标见 [`../../docs/bidding/authoring.md`](../../../docs/bidding/authoring.md)。下文「六类现有业务 target」描述当前仍在跑的 V1 任务；V2 编制 job 按 [`../tender-to-submission-v2.md`](../tender-to-submission-v2.md) 接入同一 Oxana 合同。Oxana 负责 enqueue、retry、retry delay、并发消费、worker crash resurrection 和 dead queue；招投标不复制这些能力。
...
1. 固化 [`../platform/queue-runtime.md`](../../../plans/platform/queue-runtime.md) 的 Oxana 2.1.3 合同；
