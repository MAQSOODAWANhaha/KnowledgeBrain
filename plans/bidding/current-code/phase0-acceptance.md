...
## Confirmed continuation baseline

Phase 0 is closed and is not the continuing implementation center. Subsequent work follows the single user-visible flow in [`../tender-to-submission-v2.md`](./tender-to-submission-v2.md) and [`../frontend-authoring.md`](./frontend-authoring.md) (contract §2.4):

```text
files → authoring (Word-style outline + Tiptap canvas) → export
Upload tender files → parse → generate/edit outline → fill from knowledge base → export bid document
```

Continuation rules:

- Reopen Phase 0 only when a concrete defect blocks this flow; do not extend Phase 0 for speculative contract completeness.
- Phase 1 first completes one usable upload-and-parse slice, then freezes the selected document set and moves directly into outline generation.
- Manifest, queue/cutover fixtures and broad negative matrices remain maintenance evidence rather than user-facing milestones.
- New abstractions, state machines and identities require a direct need in the five-step flow.
- Lint findings must be fixed structurally; arbitrary `#[allow(...)]` attributes are not accepted.
...
- SubmissionExport request mode options use exactly `watermark`, enforce JSON null/string type and a 1–128 byte draft watermark, and require submission watermark null. Valid review-draft and submission requests pass; extra/missing keys, wrong type, empty draft watermark and a submission watermark fail.
