...
知识库管线已与 WeKnora/brain **同构**（`docs/system-design.md`：oxana 任务、`parse_status`、`crates/chunker` auto/heading/heuristic/legacy + parent-child、`crates/docparser` ReadStream / MinerU·Paddle 客户端）。
...
- `docs/system-design.md` / `docs/bid-platform-domain.md` — 一句：投标抽取按段、覆盖率可见
...
对照基线：`docs/weknora-parse-extract-baseline.md`（main `ba03be77` / v0.7.2 `3d5d8bfc`）。
