...
- 首发：[部署说明](../deploy/README.md)；性能：[pgvector 边界](knowledge-base/README.md#向量检索优化边界)。两条工作独立于 O0，不要求先重做平台；知识库其他产品计划不并入。
...
S3 官方 SDK 替换已由 `s3_sdk` 完成：`aws-sdk-s3 1.146.0`，平台 72 passed/3 ignored、专用 MinIO 4 passed、平台 Clippy 通过；本机证据 `/tmp/kb-s3-sdk-52g29iye/` 待归档。该局部结果不等于全仓或 Rig 验收，不重复实施。
...
| V1 | 条件式 pgvector 分域基准 | 明确性能需求、代表性数据与 SLO | 条件未触发，不挡 O0 或正确性修复 |
...
- **可证实验收：**
  ...
  5. `consume.rs` 仍共享既定 absolute cleanup deadline，terminal write/tracker/取消与 join 不获新预算；失败/取消不遗留超期等待或 late write。
...
- **归属 / 候选文件面：** `crates/platform/tests/catalog_manifest_postgres.rs`、`crates/bidding/tests/` 既有 AgentRun/Request/检索 suite、`crates/worker/src/consume.rs`、`crates/retention/src/main.rs`、`scripts/fresh_schema_acceptance.sh`、`.github/workflows/ci.yml`；发现缺陷才按父限定范围修改。
...
- **归属 / 候选文件面：** `crates/api/src/bid_v2_routes.rs`、`crates/bidding/src/bid_authoring_v2.rs`、`crates/worker/src/consume.rs`、`web/src/bid/authoring/ExportPane.tsx`、`session.ts`。
...
- **归属 / 候选文件面：** `crates/bidding/src/render_v2.rs`、`bid_authoring_v2.rs`、`crates/worker/src/consume.rs`、`crates/api/src/bid_v2_routes.rs`、`web/src/bid/authoring/ExportPane.tsx` 及既有导出/报告测试。
...
- **归属 / 候选文件面：** `crates/bidding/src/quote_snapshot.rs`、`render_v2.rs`、`content_runtime.rs`、`bid_authoring_v2.rs`、`crates/worker/src/consume.rs`、`web/src/bid/authoring/ExportPane.tsx` 及实际文档回归。
...
- **归属 / 候选文件面：** Knowledge；`crates/knowledge/src/knowledge_retrieval_pg/semantic_v2.rs`、`crates/knowledge/src/persist.rs` 与所属索引/检索测试；本任务不纳入知识库其他产品计划。
