 ...
 * 保留，只填空着的章。
 */
export function FillPane({ workspaceId }: { workspaceId: string }) {
  ...
  return <section className="card stack" data-testid="docx-fill">
    ...
    <p className="text-sm">
      按当前 Word 的章节结构逐章填写模板正文。已经写了内容的章原样保留，只填空着的章。
    </p>
    <p className="text-sm">
    ...
    <p role="status" aria-live="polite" className="text-sm" data-testid="docx-fill-progress">
      {state.phase === "loading" ? "正在读取填充状态…" : fillProgressText(state.job, state.stopping)}
    </p>
